...
export const Calculator: React.FC = () => {
  ...
  const estimate = useMemo(() => {
    ...
    // 2025 Estimated construction costs for DIY/Contractor mix
    switch (finishLevel) {
      case 'budget':
        baseCostPerSqFt = 125;
        break;
  ...
  return (
    <div className="bg-white rounded-lg shadow-xl border border-slate-200 overflow-hidden max-w-2xl mx-auto">
      <div className="bg-industrial-gray p-4 text-white flex items-center gap-2">
        ...
        <h3 className="font-bold text-lg">Engineer's Materials Estimator</h3>
      </div>
      ...
      <div className="p-6 md:p-8 space-y-8">
        ...
        <div>
          <div className="flex justify-between mb-2">
            <label className="font-semibold text-slate-700">Tiny House Size</label>
            <span className="font-bold text-safety-orange text-xl">{sqFt} sq. ft.</span>
          </div>
          <input
            type="range"
            min="100"
        ...
        <div>
          ...
          <p className="text-sm text-slate-500 mt-2 italic">
            {finishLevel === 'budget' && "Basic materials, DIY labor, laminate counters."}
            {finishLevel === 'standard' && "Quality siding, contractor help, butcher block."}
            {finishLevel === 'luxury' && "High-end finishes, full contractor, quartz/stone."}
          </p>
        </div>

        {/* Output */}
        <div className="bg-slate-900 rounded-lg p-6 text-center">
          <p className="text-slate-400 text-sm mb-1">Estimated Material & Build Cost Range</p>
          ...
          <p className="text-xs text-slate-500 mt-3 border-t border-slate-700 pt-3">
            *Estimates vary by location. Does not include land cost or permits. 
            <br/> 
            <span className="text-safety-orange font-bold">Save ~30% by using our Engineer-Optimized Cut Lists.</span>
          </p>
        </div>
      </div>
    </div>
